...
// GET all Thoughts
  ...
  createThought(req, res) {
    ...
    .then((user) => 
        !user
            ? res.status(404).json({ message: 'Thought created, but User not found with that Id'})
  ...
  deleteThought(req, res) {
    Thought.findOneAndRemove({ _id: req.params.thoughtId })
        ...
        .then((user) =>
            !user
                ? res.status(404).json({ message: 'Thought deleted successfully but user not found with that Id' })
